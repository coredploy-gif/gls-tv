  ...
  const Section = ({
    ...
    <section className="gls-admin-card rounded-xl p-5">
      ...
      <div className="mt-4 space-y-2">
        {rows.map((r) => (
          <div
            key={`${r.user_id}-${r.status}-${r.current_period_end}`}
            className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/[0.06] bg-black/35 px-3 py-3"
          >
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-gls-muted">
                ...
                {r.current_period_end
                  ...
                  : ""}
                {r.amount_zar_cents
                  ? ` · R${(r.amount_zar_cents / 100).toFixed(0)}`
                  : ""}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                onClick={() => void remind(r.user_id, kind)}
                className="rounded-md border border-white/20 px-3 py-1.5 text-xs text-gls-body hover:text-white disabled:opacity-40"
              >
                Remind
              </button>
              {r.stripe_customer_id && (
                <button
                  type="button"
                  onClick={() => void portal(r.stripe_customer_id!)}
                  className="rounded-md border border-emerald-500/30 px-3 py-1.5 text-xs text-emerald-200 hover:bg-emerald-500/10"
                >
                  Portal
                </button>
              )}
              {r.external_id && kind === "past_due" && (
                <button
                  type="button"
                  onClick={() => void cancel(r.external_id!)}
                  className="rounded-md border border-gls-red/40 px-3 py-1.5 text-xs text-red-200 hover:bg-gls-red/10"
                >
                  Cancel
                </button>
              )}
        ...
        {!rows.length && (
          <p className="py-10 text-center text-sm text-gls-muted">Queue empty</p>
        )}
      </div>
    </section>
  );
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        description="Daily past-due chase and upcoming renewals — remind in-app, open Stripe portal, or cancel."
      />
